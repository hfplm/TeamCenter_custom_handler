...
int attachDatasetAsTarget(EPM_action_message_t msg) {
	...
	char* argValue;
	char* type = NULL;
	char* attachment_type = NULL;
	char* name = NULL;
	char* relation = NULL;
	int attachmentType[1] = {EPM_reference_attachment};
	...
	if (count) {
		...
		for (int i = 0; i < numOfArgs; i++) {
			...
			if (tc_strcmp(argName, "attachment_type") == 0) {

				attachment_type = (char*)MEM_alloc(100);
				tc_strcpy(attachment_type, "");
				tc_strcpy(attachment_type, argValue);

				if(tc_strcmp(attachment_type, "target")==0) attachmentType[0] = EPM_target_attachment;
				if(tc_strcmp(attachment_type, "reference")==0) attachmentType[0] = EPM_reference_attachment;
			}
	...
	for (int i = 0; i < count; i++) {
		...
		if (tc_strcmp(objType, "ItemRevision") == 0) {
			...
			EPM_add_attachments(rootTask, 1, &dataset, attachmentType);
		}
	}
